...
// This can not be optimized easily and be performent at the same time.

template<bool LastDelimiter = false, size_t N>
std::string_view concatenateBufferDelimiter(std::array<char, N> &buffer, std::string_view separator,
					std::string_view const ix0
		){

	if constexpr(LastDelimiter)
...
#endif // concatenateBufferDelimiter

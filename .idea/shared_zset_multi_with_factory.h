...
namespace net::worker::shared::zsetmulti{
	...
	namespace impl_{
		...
		void add_ctrlKey_(DBAdapter &db,
	...
	void add(DBAdapter &db,
		...
		{
			add_ctrlKey_<Permutation, IndexController>(db, keyCtrl, keyN, keySub, indexes);
		}
	...
	void addF(DBAdapter &db,
		...
		{
			using IndexController = std::nullptr_t;
		...
		auto insertNewKeys = [&](std::string_view key){
			...
			hm4::insertF<hm4::PairFactory::Normal>(*db, key, value);
		};
		...
		auto const encodedValue = Permutation::encodeIndex(bufferVal, DBAdapter::SEPARATOR, indexes);

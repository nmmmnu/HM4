...
namespace net::worker::commands::RB{
		...
		void process(ParamContainer const &p, DBAdapter &db, Result<Protocol> &result, OutputBlob &) final{
			...
			auto f = [&](auto x) {
				...
				if constexpr(std::is_same_v<T, std::nullptr_t>){
					...
					return process__(rb, key, p, *db, result);
				}
			};
	...
	private:
		...
		static void process__(MyRingBuffer const &rb, std::string_view const key, ParamContainer const &p, typename DBAdapter::List &list, Result<Protocol> &result){
			auto const varg = 4;
		...
		void process(ParamContainer const &p, DBAdapter &db, Result<Protocol> &result, OutputBlob &) final{
			...
			auto f = [&](auto x) {
				...
				if constexpr(std::is_same_v<T, std::nullptr_t>){
					...
					return process__(rb, key, *db, result);
				}
			};
	...
	private:
		template<typename MyRingBuffer>
		static void process__(MyRingBuffer const &rb, std::string_view const key, typename DBAdapter::List &list, Result<Protocol> &result){
			const auto *pair = hm4::getPairPtrWithSize(list, key, rb.bytes());
		...
		void process(ParamContainer const &p, DBAdapter &db, Result<Protocol> &result, OutputBlob &blob) final{
			...
			auto f = [&](auto x) {
				...
				if constexpr(std::is_same_v<T, std::nullptr_t>){
					...
					return process__(rb, key, *db, result, blob);
				}
			};
	...
	private:
		...
		static void process__(MyRingBuffer const &rb, std::string_view const key, typename DBAdapter::List &list, Result<Protocol> &result, OutputBlob &blob){
		//	const auto *pair = hm4::getPairPtrWithSize(list, key, rb.bytes());
			...
			if (pair->getValLen() != rb.bytes())
				return process_SL__(pair, result, blob);
			else
				return process_RB__(rb, pair, result, blob);
		}

		static void process_SL__(const hm4::Pair *pair, Result<Protocol> &result, OutputBlob &blob){
			auto &container = blob.construct<OutputBlob::Container>();

			using RawSList = s_list::RawSListConst;

			auto const slist = RawSList{
							hm4::getValAs<RawSList::List>(pair),
							pair->getValLen()
			};

			auto f = [&container](const auto &item){
			...
			// this may return value bigger than slots,
			// if run on SL Object, but this is OK.
			return result.set_container(container);
		}

		template<class MyRingBuffer>
		static void process_RB__(MyRingBuffer const &rb, const hm4::Pair *pair, Result<Protocol> &result, OutputBlob &blob){
			auto &container = blob.construct<OutputBlob::Container>();
	...
	private:
		...
		//	const auto *pair = hm4::getPairPtrWithSize(list, key, rb.bytes());
			...
			if (pair->getValLen() != rb.bytes())
				return process_SL__(pair, result);
			else
				return process_RB__(rb, pair, result);
		}

		static void process_SL__(const hm4::Pair *pair, Result<Protocol> &result){
			using RawSList = s_list::RawSListConst;
			...
			// this may return value bigger than slots,
			// if run on SL Object, but this is OK.
			return result.set(uint64_t{ slist.size() });
		}

		template<class MyRingBuffer>
		static void process_RB__(MyRingBuffer const &rb, const hm4::Pair *pair, Result<Protocol> &result){
			using List = typename MyRingBuffer::List;

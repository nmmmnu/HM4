...
namespace net::worker::commands::MG{
	...
	struct MGADD : BaseRW<Protocol,DBAdapter>{
		...
		// MGADD key slots size bytes item item
	...
	private:
		...
		void process_(MyRawMisraGries &mg, std::string_view key, ParamContainer const &p, typename DBAdapter::List &list, Result<Protocol> &result) const{
			auto const varg = 4;

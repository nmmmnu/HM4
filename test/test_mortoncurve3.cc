...
namespace{
	...
	size_t search(	IT begin, IT const end,
		...
		while(it != end){
			...
			if constexpr(BigMinOptimized)
				printf("%6" PRIu64 " %4" PRIu32 " %4" PRIu32 " %4" PRIu32 " %s\n", (uint64_t) zzz, x, y, z, s);
...
int main(){
	...
	struct P{
		...
		constexpr P(uint128_t zzz, uint32_t x, uint32_t y, uint32_t z) : zzz(zzz), x(x), y(y), z(z){}
	...
	if constexpr(0){
		...
		for(auto const &[zzz, x, y, z] : container){
			printf("%6" PRIu64 " %4" PRIu32 " %4" PRIu32 " %4" PRIu32 "\n", (uint64_t) zzz, x, y, z);
		}
	}

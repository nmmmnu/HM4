...
namespace{
	...
	size_t search(	IT begin, IT const end,
			std::array<uint8_t, 8> const v_min,
			std::array<uint8_t, 8> const v_max
		){
...
int main(){
	[[maybe_unused]] constexpr std::array<uint8_t, 8> v_min{ 0,1,0,1,  0,1,0,1 };
	[[maybe_unused]] constexpr std::array<uint8_t, 8> v_max{ 1,2,1,2,  1,2,1,2 };

	struct P{
		uint64_t zzz;
		std::array<uint8_t, 8> v;

		constexpr P(uint64_t zzz, std::array<uint8_t, 8> const &v) : zzz(zzz), v(v){}

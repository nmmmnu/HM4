...
namespace{
	...
	const Pair *raw_memory_system(){
		static const uint8_t blob[] = {
			0x00, 0x00, 0x00, 0x00,			// created, 2012-07-13 11:01:20
			0x00, 0x00, 0x00, 0x00,			// milliseconds
			0xFF, 0xFF, 0xFF, 0xFF,			// expires
			0x00, 0x00, 0x00, 0x00,			// vallen
			0x00, 0x05,				// keylen
			'\0', '\0', 'S', 'Y', 'S', '\0',	// key
			'\0'					// val
		};
	...
	void pair_test_raw_system(){
		mytest.begin("Pair Raw System");

		std::string_view const key{ "\0\0" "SYS", 5 };

		const Pair *p = raw_memory_system();

		mytest("system",	p->isSystemPair()		);
		mytest("expired",	p->isExpired()			);

		mytest("key",		p->getKey() == key		);
		mytest("val",		p->getVal().empty()		);
